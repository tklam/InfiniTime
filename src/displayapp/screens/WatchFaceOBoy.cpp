#include "displayapp/screens/WatchFaceOBoy.h"
...
WatchFaceOBoy::WatchFaceOBoy(Controllers::DateTime& dateTimeController,
  ...
  lfs_file f = {};
  if (filesystem.FileOpen(&f, "/fonts/lv_font_dots_36.bin", LFS_O_RDONLY) >= 0) {
    filesystem.FileClose(&f);
    font_dot40 = lv_font_load("F:/fonts/lv_font_dots_36.bin");
  }
  ...
  if (filesystem.FileOpen(&f, "/fonts/7segments_76.bin", LFS_O_RDONLY) >= 0) {
    filesystem.FileClose(&f);
    font_segment76 = lv_font_load("F:/fonts/7segments_76.bin");
  }

  background_image = lv_img_create(lv_scr_act(), nullptr);
  lv_img_set_src(background_image, "F:/images/o_boy.bin");
  lv_obj_set_pos(background_image, 0, 10);
  ...
  lv_obj_align(label_day_of_week, lv_scr_act(), LV_ALIGN_IN_TOP_LEFT, 100, 30);
  ...
  */
  ...
  lv_obj_set_style_local_text_font(label_time, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, font_segment76);
...
WatchFaceOBoy::~WatchFaceOBoy() {
  ...
  if (font_segment76 != nullptr) {
    lv_font_free(font_segment76);
  }

  lv_obj_clean(lv_scr_act());
}

void WatchFaceOBoy::Refresh() {
  ...
  if (currentDateTime.IsUpdated()) {
    ...
    if (currentDate.IsUpdated()) {
      ...
      if (settingsController.GetClockType() == Controllers::Settings::ClockType::H24) {
        ...
        lv_label_set_text_fmt(label_date, "%3d %s", day, dateTimeController.MonthShortToString());
      ...
      */
      ...
      lv_label_set_text_fmt(label_day_of_week, "%s%4d", dateTimeController.DayOfWeekShortToString(), year);
...
bool WatchFaceOBoy::IsAvailable(Pinetime::Controllers::FS& filesystem) {
  lfs_file file = {};

  if (filesystem.FileOpen(&file, "/fonts/lv_font_dots_36.bin", LFS_O_RDONLY) < 0) {
    return false;
  }
  ...
  filesystem.FileClose(&file);
  if (filesystem.FileOpen(&file, "/fonts/7segments_76.bin", LFS_O_RDONLY) < 0) {
    return false;
  }
